
class @main extends @template 'lp/all'
  route : '/repetitory-biologiya-ege'
  #tags  : -> ['registration']
  model : 'main/registration'
  title : "Репетиторы по биологии ЕГЭ"
  tree : =>
    photo_src         : 'reclame_jump_page/land_biology.jpg'
    alt               : 'Репетиторы по биологии ЕГЭ'
    title             : 'Для чего нужен репетитор по биологии?'
    #sub_title         : 'подготовка к ЕГЭ'
    text              : '<p><b>ЕГЭ по биологии</b> необходим школьникам, которые хотят обучаться специальностям медицинского направления.</p> <p>С каждым годом этот экзамен становится сложнее: исключаются тестовые задания с несколькими вариантами решения на выбор, становится больше вопросов с требованием развернутого полного ответа. Также попыток хорошо сдать экзамен стало больше — теперь каждый школьник может попробовать набрать нужный балл три раза.</p> <p><i>Ознакомиться с особенностями сдачи ЕГЭ по биологии, а также с пробными заданиями вы можете <a href="http://www.examen.ru/add/ege/ege-po-biologii" target="_blank">здесь</a></i>.</p> <p>Если мечта вашего ребенка — высшее медицинское образование, вам обязательно стоит найти хорошего репетитора по биологии. Благодаря его усилиям, школьник получит знания, которые позволят набрать нужные баллы и стать студентом выбранного университета в Москве.</p> <p><b>Наша база профессиональных педагогов — лучший вариант для поиска наставника, организующего должный уровень подготовки.</b> </p> <p>Подбор репетитора абсолютно бесплатно!</p>'
    title_suit_tutors : 'Репетиторы для подготовки к ЕГЭ по биологии.'
    landing_img       : 'reclame_background/biolog.png'
    title_position    : 'top'
    filter :
      subject : [
        'биология'
      ]
