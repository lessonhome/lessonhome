class @main
  tree : -> module '$' :
    ...
    course     : module 'tutor/forms/drop_down_list' :
      ...
      value: data('tutor').get('subjects').then (s)-> return s.tags[0] if s.tags?[0]?
    ...
    price_slider   : state 'main/slider_main' :
      ...
      value         :
        min : 400
        max : 5000
        left : data('tutor').get('subjects').then (s)-> return s.price[0].range[0] if s?.price?[0]?.range?[0]?
        right : data('tutor').get('subjects').then (s)-> return s.price[0].range[1] if s?.price?[0]?.range?[1]?
