...
class Interface
  ...
  init : => @locker.$lock =>
    ...
    yield @main.jobs.listen 'pupilReceiveBid',(data)=> @main.bids.receiveBid data

    yield @main.jobs.client 'pupilChatPush',(auth,hash,msg)=> @main.chats.msgPush auth,hash,msg
    
    yield @main.jobs.client 'pupilUpdateBid',(auth,index,data)=> @main.bids.bidUpdate auth,index,data
