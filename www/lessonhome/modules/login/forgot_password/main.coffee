...
class @main extends EE
  ...
  showError : (err)=>
    switch err
      ...
      when 'limit_attempt'
        @login.showError 'Сообщение все ещё не пришло?  Для восстановления доступа позвоните нам по телефону +7 (495) 181-03-73'
      when 'send_later'
        Feel.go '/send_code'
#        @login.showError 'Сообщение уже отправлено. Пожалуйста, повторите попытку позже'
      when 'error_sms'
